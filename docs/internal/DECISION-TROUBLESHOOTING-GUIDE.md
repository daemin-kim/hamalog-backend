...
2. [개요](#1-개요)
3. [Phase 1: ADR - 구현 전/중 의사결정 기록](#2-phase-1-adr---구현-전중-의사결정-기록)
4. [Phase 2: 트러블슈팅 로그 - 문제해결 수사 일지](#3-phase-2-트러블슈팅-로그---문제해결-수사-일지)
5. [Phase 3: 문서 패키징 및 공유](#4-phase-3-문서-패키징-및-공유)
6. [파일 위치 및 네이밍 규칙](#5-파일-위치-및-네이밍-규칙)
...
#### ADR 작성
```bash
# 새 ADR 생성 (번호는 docs/internal/adr/ 내 다음 순번 사용)
cp docs/internal/adr/ADR-TEMPLATE.md docs/internal/adr/ADR-XXXX-{제목}.md
```
...
# 예시
cp docs/troubleshooting/TSL-0001-template.md docs/troubleshooting/TSL-0002-redis-connection-timeout.md
```
...
├── troubleshooting/      # 트러블슈팅 로그 (TSL)
│   ├── README.md         # 가이드 및 목록
│   └── TSL-0001-template.md  # 템플릿
