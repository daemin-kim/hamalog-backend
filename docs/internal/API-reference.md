...
> 프론트엔드 개발자는 [API 명세서](../shared/API-specification.md)를 참조하세요.

## 프로젝트 문서

| 문서 | 설명 | 대상 |
|------|------|------|
| [API 명세서](../shared/API-specification.md) | REST API 엔드포인트 상세 명세 | 프론트엔드 개발자 |
| [API 참고 문서](./API-reference.md) | 현재 문서 - 인프라, 스키마, 변경 이력 | 백엔드/DevOps |
| [프로젝트 구조 설명서](../shared/Project-Structure.md) | 프로젝트 디렉토리 구조, 아키텍처, 배포 구성 | 전체 |
| [바이브 코딩 가이드](../ai/VIBE-CODING-GUIDE.md) | AI 협업 개발 가이드, 바이브 코딩 전략 | AI/개발자 |
| [코딩 컨벤션](./CODING-CONVENTIONS.md) | 코드 스타일 및 규칙 | 개발자 |
| [ADR 문서](./adr/) | 아키텍처 결정 기록 | 개발자 |
| [향후 개선사항](../shared/FUTURE-IMPROVEMENTS.md) | 기능 구현 현황 및 로드맵 | 전체 |
...
> 📖 상세한 배포 아키텍처 및 CI/CD 파이프라인 정보는 [프로젝트 구조 설명서](../shared/Project-Structure.md#배포-아키텍처)를 참조하세요.
...
- **2025/12/29**: **코드 정리 및 보안 기능 통합**
    - **GlobalExceptionHandler 보안 기능 강화**:
        - `SecureErrorHandler` 기능을 `GlobalExceptionHandler`로 병합
        - 보안 로깅 (SECURITY 로거), 클라이언트 IP 추출, 입력값 sanitization
        - `AccessDeniedException`, `MaxUploadSizeExceededException` 등 핸들러 추가
    - **미사용 코드 삭제**:
        - `SecureErrorHandler.java` (중복 핸들러)
        - `SecureFileStorageService.java` (미사용)
        - `SafeHttpClientUtil.java` (미사용)
